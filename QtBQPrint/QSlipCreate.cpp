...
int QSlipCreate::GenerateSlipInfo(SLIP_PRINT_INFO slip_info, cv::Mat& img)
{
	...
	putTextZH(m_SlipImg, Couchar, cv::Point(440, 280), cv::Scalar(0, 0, 0), 40, "Arial", false, true);
	array = Total_amount.toLocal8Bit();
	char *Totalchar = array.data();
	putTextZH(m_SlipImg, Totalchar, cv::Point(420, 350), cv::Scalar(0, 0, 0), 80, "Arial");
	array = NumCoupons.toLocal8Bit();
	char *NumCouponschar = array.data();
	putTextZH(m_SlipImg, NumCouponschar, Numpoint, cv::Scalar(0, 0, 0), 80, "Arial");
	array = slip_info.date.toLocal8Bit();
	char *datechar = array.data();
	putTextZH(m_SlipImg, datechar, cv::Point(30, 565), cv::Scalar(0, 0, 0), 40, "Arial");
	if (!slip_info.FHInfo.isEmpty())
	{
		array = slip_info.FHInfo.toLocal8Bit();
		char *FHInfochar = array.data();
		putTextZH(m_SlipImg, FHInfochar, cv::Point(530, 565), cv::Scalar(0, 0, 0), 40, "Arial");
	}
	if (!slip_info.FQInfo.isEmpty())
	{
		array = slip_info.FQInfo.toLocal8Bit();
		char *FQInfochar = array.data();
		putTextZH(m_SlipImg, FQInfochar, cv::Point(950, 565), cv::Scalar(0, 0, 0), 40, "Arial");
	}
